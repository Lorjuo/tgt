...
class TimepickerInput < SimpleForm::Inputs::FileInput
  def input
    ...
    icon = template.tag(:span, :class => "glyphicon glyphicon-time")
    append = template.content_tag(:span, icon, :class => "input-group-addon")

    wrapper = template.content_tag(:div, input + append, :class => "input-group bootstrap-timepicker")
  end
end